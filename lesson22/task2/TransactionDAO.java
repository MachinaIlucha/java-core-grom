...
//        - если город оплаты (совершения транзакции) не разрешен
    ...
    private static void checkSize(Transaction transaction) throws InternalServerException {
        int nulls = 0;
        for (Transaction transaction1 : transactions) {
            if (transaction1 == null)
                nulls++;
    ...
    private static void checkCityOfTran(Transaction transaction) throws BadRequestException {
        boolean c = false;
